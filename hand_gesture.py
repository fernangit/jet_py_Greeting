...
def display_landmark(hand_landmarks):
    for i, lm in enumerate(hand_landmarks.landmark):
        height, width, channel = img.shape
        cx, cy = int(lm.x * width), int(lm.y * height)
        cv2.putText(img, str(i + 1), (cx + 10), cv2.FONT_HERSHEY_PLAIN, 4, (255, 255, 255), 5, cv2.LINE_AA)
...
def detect_hand_gesture(img):
    #BGR -> RGB
    imgRGB = cv2.cvColor(img, cv2.COLOR_BGR2RGB)
    ...
    if results.multi_hand_landmarks:
        for hl, hand_landmarks in enumerate(results.multi_hand_landmarks):
            display_landmark(hand_landmarks)

            if hl < 2:
                get_landmark(hand_landmarks, hl)
                #指の第一関節と手のひらとの距離
                fdis = relative_distance(p[hl], fingebot)
                avefdis = sum(fdis) / len(fdis)
                fdis = relative_distance(p[hl], fingetop)
                fdis = [(x / avefdis) * 100 for x in fdis]
                print(hl, fdis[0], fdis[1], fdis[2], fdis[3], fdis[4])

                #thumbs up検出
                if detect_thumbs(fdis) == True and detect_thumbs_updown(p[hl])  == True:
                    print('thumbs up!')
                    return H_THUMBS_UP
                elif detect_thumbs(fdis) == True and detect_thumbs_updown(p[hl]) == False:
                    print('thumbs down!')
